...
{
    /// <summary>
    /// Maneja la lógica de UI del IotSettings, incluyendo paneles modales y menú lateral
    /// Equivalente a WelcomeUIManager pero especializado para IotSettings
    /// </summary>
    public class IotSettingsUIManager
    {
        private IotSettingsInfo.UIConfiguration _uiConfig;
        private IIotSettingsOps.PanelType _currentActivePanel = IIotSettingsOps.PanelType.None;
        ...
        public IotSettingsUIManager(IotSettingsInfo.UIConfiguration uiConfig)
        {
        ...
        /// <summary>
        /// Inicializa el sistema de paneles del IotSettings
        /// </summary>
        public void InitializePanelSystem()
        {
            // Asegurar estado inicial correcto
            _uiConfig.SubpanelsContainer.style.display = DisplayStyle.None;
            HideUi(); // IotSettings inicia oculto hasta autenticación
            ClearAllPanelStates();
            RegisterTransitionCallbacks();
            
            Debug.Log("[IotSettingsUIManager] Panel system initialized");
        }

        /// <summary>
        /// Muestra la UI principal del IotSettings
        /// </summary>
        public void ShowUi()
        {
            _uiConfig.Body.style.display = DisplayStyle.Flex;
            Debug.Log("[IotSettingsUIManager] IotSettings UI shown");
        }

        /// <summary>
        /// Oculta la UI principal del IotSettings
        /// </summary>
        public void HideUi()
        {
            ...
            CloseAllPanels();
            Debug.Log("[IotSettingsUIManager] IotSettings UI hidden");
        }
        ...
        public void ShowNavigationMenu()
        {
            ...
            if (!_uiConfig.Panels.TryGetValue(IIotSettingsOps.PanelType.NavigationMenu, out var menuPanel))
            {
            ...
            _currentActivePanel = IIotSettingsOps.PanelType.NavigationMenu;
            _isNavigationMenuOpen = true;
    
            Debug.Log("[IotSettingsUIManager] Navigation menu opened");
        }
        ...
        public void HideNavigationMenu()
        {
            ...
            _currentActivePanel = IIotSettingsOps.PanelType.None;
            _isNavigationMenuOpen = false;
    
            Debug.Log("[IotSettingsUIManager] Navigation menu closed");
        }
        ...
        /// </summary>
        public void ShowPanel(IIotSettingsOps.PanelType panelType)
        {
            if (panelType == IIotSettingsOps.PanelType.NavigationMenu)
            {
                ShowNavigationMenu();
                return;
            }
            ...
            if (_currentActivePanel != IIotSettingsOps.PanelType.None && 
                _currentActivePanel != IIotSettingsOps.PanelType.NavigationMenu)
            {
            ...
            Debug.Log($"[IotSettingsUIManager] Panel {panelType} opened");
        }

        /// <summary>
        /// Oculta un panel específico
        /// </summary>
        public void HidePanel(IIotSettingsOps.PanelType panelType)
        {
            if (panelType == IIotSettingsOps.PanelType.NavigationMenu)
            {
                HideNavigationMenu();
            ...
            Debug.Log($"[IotSettingsUIManager] Panel {panelType} closed");
        }
        ...
        {
            if (fromPanel != IIotSettingsOps.PanelType.None)
            {
                HidePanel(fromPanel);
            }
            
            if (toPanel != IIotSettingsOps.PanelType.None)
            {
                ShowPanel(toPanel);
            }
        }
        ...
        /// </summary>
        private bool HasOtherPanelsRequiringScrim(IIotSettingsOps.PanelType excludePanel)
        {
            foreach (var kvp in _uiConfig.Panels)
            {
        ...
        private void UpdateSingleStatusLabel(Label statusLabel, Label modeLabel, 
            IotSettingsInfo.ConnectionStatus status, string prefix)
        {
            ...
            switch (status)
            {
                case IotSettingsInfo.ConnectionStatus.Connected:
                    statusLabel.AddToClassList("status-connected");
                    break;
                case IotSettingsInfo.ConnectionStatus.Disconnected:
                    statusLabel.AddToClassList("status-disconnected");
                    break;
                case IotSettingsInfo.ConnectionStatus.Error:
                    statusLabel.AddToClassList("status-error");
                    break;
                case IotSettingsInfo.ConnectionStatus.Connecting:
                    statusLabel.AddToClassList("status-connecting");
                    break;
            }
        }

        private string GetStatusText(IotSettingsInfo.ConnectionStatus status)
        {
            return status switch
            {
                IotSettingsInfo.ConnectionStatus.Connected => "Connected",
                IotSettingsInfo.ConnectionStatus.Connecting => "Connecting...",
                IotSettingsInfo.ConnectionStatus.Disconnected => "Disconnected",
                IotSettingsInfo.ConnectionStatus.Error => "Error",
                _ => "Unknown"
            };
        }

        private string GetModeText(IotSettingsInfo.ConnectionStatus status)
        {
            return status switch
            {
                IotSettingsInfo.ConnectionStatus.Connected => "Active",
                IotSettingsInfo.ConnectionStatus.Connecting => "Initializing",
                IotSettingsInfo.ConnectionStatus.Disconnected => "Inactive",
                IotSettingsInfo.ConnectionStatus.Error => "Failed",
                _ => "Unknown"
            };
        }
        ...
        private void ClearAllPanelStates()
        {
            foreach (var kvp in _uiConfig.Panels)
            {
                ...
                if (panelData.Panel != null)
                {
                    ...
                    if (kvp.Key != IIotSettingsOps.PanelType.NavigationMenu)
                    {
        ...
        private void OnPanelTransitionComplete(TransitionEndEvent evt)
        {
            ...
            {
                _uiConfig.SubpanelsContainer.style.display = DisplayStyle.None;
                Debug.Log("[IotSettingsUIManager] All panels closed - hiding container");
            }
        }
        ...
        /// </summary>
        public IIotSettingsOps.PanelType CurrentActivePanel => _currentActivePanel;
