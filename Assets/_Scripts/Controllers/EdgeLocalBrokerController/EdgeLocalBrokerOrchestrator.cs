...
{
    /// <summary>
    /// Coordinador principal del EdgeLocalBroker - implementa IEdgeLocalBrokerOps e IUIController
    /// Equivalente a WelcomeOrchestrator pero para EdgeLocalBroker
    /// </summary>
    public class EdgeLocalBrokerOrchestrator : MonoBehaviour, IEdgeLocalBrokerOps, IUIController
    {
        #region IUIController Implementation

        public bool RequiresAuthentication => true; // EdgeLocalBroker SÍ requiere autenticación
        public bool IsInitialized => _isInitialized;
        public bool IsActive => _uiConfig?.Body?.style.display == DisplayStyle.Flex;
        public string ControllerName => "EdgeLocalBrokerController";
        ...
        #region IEdgeLocalBrokerOps Implementation

        public bool IsNavigationMenuOpen => _uiManager?.NavigationMenuOpen ?? false;
        public IEdgeLocalBrokerOps.PanelType CurrentActivePanel => _uiManager?.CurrentActivePanel ?? IEdgeLocalBrokerOps.PanelType.None;

        // Events from IEdgeLocalBrokerOps
        public event Action OnNavigationMenuOpened;
        public event Action OnNavigationMenuClosed;
        public event Action<IEdgeLocalBrokerOps.PanelType> OnPanelTransitionComplete;
        public event Action OnLogoutRequested;
        public event Action<EdgeLocalBrokerInfo.ConnectionStatus, EdgeLocalBrokerInfo.ConnectionStatus, EdgeLocalBrokerInfo.ConnectionStatus> OnIoTStatusChanged;

        #endregion

        #region Private Fields

        private EdgeLocalBrokerInfo.UIConfiguration _uiConfig = new EdgeLocalBrokerInfo.UIConfiguration();
        private EdgeLocalBrokerInfo.UserData _userData = new EdgeLocalBrokerInfo.UserData();
        private EdgeLocalBrokerInfo.EdgeLocalBrokerState _EdgeLocalBrokerState = new EdgeLocalBrokerInfo.EdgeLocalBrokerState();
        
        private EdgeLocalBrokerUIManager _uiManager;
        private EdgeLocalBrokerEventManager _eventManager;
        ...
        private void Start()
        {
            // CRÍTICO: EdgeLocalBroker inicia OCULTO hasta autenticación
            HideUi();
            _subpanelsAndSmokeMaskContainer.style.display = DisplayStyle.None;
            
            Debug.Log("[EdgeLocalBrokerOrchestrator] Started - UI hidden until authentication");
        }
        ...
        public bool Initialize()
        {
            try
            {
                if (_isInitialized)
                {
                    Debug.Log("EdgeLocalBrokerOrchestrator already initialized");
                    return true;
                }

                Debug.Log("Initializing EdgeLocalBrokerOrchestrator...");
                ...
                _uiManager = new EdgeLocalBrokerUIManager(_uiConfig);
                _eventManager = new EdgeLocalBrokerEventManager(_uiManager, OnLogoutRequestedHandler, OnPanelTransitionCompleteHandler, this);
                ...
                // Inicializar estado
                InitializeEdgeLocalBrokerState();

                // Suscribirse a eventos de ServiceController
                SubscribeToServiceControllerEvents();

                _isInitialized = true;
                Debug.Log("✅ EdgeLocalBrokerOrchestrator initialized successfully");
                OnControllerInitialized?.Invoke(this);
                return true;
            }
            catch (Exception ex)
            {
                Debug.LogError($"EdgeLocalBrokerOrchestrator initialization error: {ex.Message}");
        ...
        public void Show()
        {
            ...
            {
                Debug.LogError("Cannot show EdgeLocalBroker - user not authenticated");
            ...
            if (_uiConfig?.Body != null)
            {
                ...
                Debug.Log("[EdgeLocalBrokerOrchestrator] EdgeLocalBroker UI shown");
            }
        }
        ...
        public void Cleanup()
        {
            ...
            {
                Debug.LogError($"[EdgeLocalBrokerOrchestrator] Cleanup error: {ex.Message}");
            }
        }
        ...
        public void NavigateToPanel(IEdgeLocalBrokerOps.PanelType panelType)
        {
        ...
        public void HandleReportsClick()
        {
            ...
            // Cerrar menú y ocultar EdgeLocalBroker
            _uiManager?.HideNavigationMenu();
            Hide();
        ...
        public void HandleOperationsClick()
        {
            var parameters = new Dictionary<string, object> {
                ["context"] = "Operations", 
                ["sourceController"] = "EdgeLocalBroker"
            };
        ...
        /// <summary>
        /// Obtiene componentes UI del EdgeLocalBroker
        /// </summary>
        private void GetUiComponents(VisualElement root)
        {
            ...
            Debug.Log("✅ EdgeLocalBroker UI components obtained successfully");
        }
...
private void InitializePanelConfiguration(VisualElement root)
{
    ...
    _uiConfig.Panels[IEdgeLocalBrokerOps.PanelType.NavigationMenu] = new EdgeLocalBrokerInfo.UIConfiguration.PanelData
    {
    ...
    _uiConfig.Panels[IEdgeLocalBrokerOps.PanelType.Notifications] = new EdgeLocalBrokerInfo.UIConfiguration.PanelData
    {
    ...
    _uiConfig.Panels[IEdgeLocalBrokerOps.PanelType.UserProfile] = new EdgeLocalBrokerInfo.UIConfiguration.PanelData
    {
        Panel = null, // 👈 SERÁ NULL porque no existe en UXML
    ...
    _uiConfig.Panels[IEdgeLocalBrokerOps.PanelType.QuickActions] = new EdgeLocalBrokerInfo.UIConfiguration.PanelData
    {
        Panel = null, // 👈 SERÁ NULL porque no existe en UXML
    ...
    _uiConfig.Panels[IEdgeLocalBrokerOps.PanelType.StatusOverlay] = new EdgeLocalBrokerInfo.UIConfiguration.PanelData
    {
        Panel = null, // 👈 SERÁ NULL porque no existe en UXML
    ...
    foreach (var kvp in _uiConfig.Panels)
    {
        ...
        private void FindDependencies()
        {
            ...
            Debug.Log("EdgeLocalBroker dependencies search completed");
        }

        /// <summary>
        /// Inicializa el estado del EdgeLocalBroker
        /// </summary>
        private void InitializeEdgeLocalBrokerState()
        {
            _EdgeLocalBrokerState.IsInitialized = true;
            _EdgeLocalBrokerState.CurrentSection = "EdgeLocalBroker";
            _EdgeLocalBrokerState.CurrentActivePanel = IEdgeLocalBrokerOps.PanelType.None;
            
            // Inicializar estados de IoT
            _EdgeLocalBrokerState.LocalIoTStatus = EdgeLocalBrokerInfo.ConnectionStatus.Unknown;
            _EdgeLocalBrokerState.VMIoTStatus = EdgeLocalBrokerInfo.ConnectionStatus.Unknown;
            _EdgeLocalBrokerState.CloudIoTStatus = EdgeLocalBrokerInfo.ConnectionStatus.Unknown;
            
            Debug.Log("EdgeLocalBroker state initialized");
        }
        ...
        /// </summary>
        private void OnPanelTransitionCompleteHandler(IEdgeLocalBrokerOps.PanelType panelType)
        {
        ...
        {
            _EdgeLocalBrokerState.LocalIoTStatus = localStatus;
            _EdgeLocalBrokerState.VMIoTStatus = vmStatus;
            _EdgeLocalBrokerState.CloudIoTStatus = cloudStatus;
        ...
        /// </summary>
        public EdgeLocalBrokerEventManager EventManager => _eventManager;

        /// <summary>
        /// Estado actual del EdgeLocalBroker
        /// </summary>
        public EdgeLocalBrokerInfo.EdgeLocalBrokerState EdgeLocalBrokerState => _EdgeLocalBrokerState;

        /// <summary>
        /// Datos del usuario actual
        /// </summary>
        public EdgeLocalBrokerInfo.UserData UserData => _userData;
