...
{
    /// <summary>
    /// Maneja la lógica de UI del ManagedCloudService, incluyendo paneles modales y menú lateral
    /// Equivalente a WelcomeUIManager pero especializado para ManagedCloudService
    /// </summary>
    public class ManagedCloudServiceUIManager
    {
        private ManagedCloudServiceInfo.UIConfiguration _uiConfig;
        private IManagedCloudServiceOps.PanelType _currentActivePanel = IManagedCloudServiceOps.PanelType.None;
        ...
        public ManagedCloudServiceUIManager(ManagedCloudServiceInfo.UIConfiguration uiConfig)
        {
        ...
        /// <summary>
        /// Inicializa el sistema de paneles del ManagedCloudService
        /// </summary>
        public void InitializePanelSystem()
        {
            // Asegurar estado inicial correcto
            _uiConfig.SubpanelsContainer.style.display = DisplayStyle.None;
            HideUi(); // ManagedCloudService inicia oculto hasta autenticación
            ClearAllPanelStates();
            RegisterTransitionCallbacks();
            
            Debug.Log("[ManagedCloudServiceUIManager] Panel system initialized");
        }

        /// <summary>
        /// Muestra la UI principal del ManagedCloudService
        /// </summary>
        public void ShowUi()
        {
            _uiConfig.Body.style.display = DisplayStyle.Flex;
            Debug.Log("[ManagedCloudServiceUIManager] ManagedCloudService UI shown");
        }

        /// <summary>
        /// Oculta la UI principal del ManagedCloudService
        /// </summary>
        public void HideUi()
        {
            ...
            CloseAllPanels();
            Debug.Log("[ManagedCloudServiceUIManager] ManagedCloudService UI hidden");
        }
        ...
        public void ShowNavigationMenu()
        {
            ...
            if (!_uiConfig.Panels.TryGetValue(IManagedCloudServiceOps.PanelType.NavigationMenu, out var menuPanel))
            {
            ...
            _currentActivePanel = IManagedCloudServiceOps.PanelType.NavigationMenu;
            _isNavigationMenuOpen = true;
    
            Debug.Log("[ManagedCloudServiceUIManager] Navigation menu opened");
        }
        ...
        public void HideNavigationMenu()
        {
            ...
            _currentActivePanel = IManagedCloudServiceOps.PanelType.None;
            _isNavigationMenuOpen = false;
    
            Debug.Log("[ManagedCloudServiceUIManager] Navigation menu closed");
        }
        ...
        /// </summary>
        public void ShowPanel(IManagedCloudServiceOps.PanelType panelType)
        {
            if (panelType == IManagedCloudServiceOps.PanelType.NavigationMenu)
            {
                ShowNavigationMenu();
                return;
            }
            ...
            if (_currentActivePanel != IManagedCloudServiceOps.PanelType.None && 
                _currentActivePanel != IManagedCloudServiceOps.PanelType.NavigationMenu)
            {
            ...
            Debug.Log($"[ManagedCloudServiceUIManager] Panel {panelType} opened");
        }

        /// <summary>
        /// Oculta un panel específico
        /// </summary>
        public void HidePanel(IManagedCloudServiceOps.PanelType panelType)
        {
            if (panelType == IManagedCloudServiceOps.PanelType.NavigationMenu)
            {
                HideNavigationMenu();
            ...
            Debug.Log($"[ManagedCloudServiceUIManager] Panel {panelType} closed");
        }
        ...
        {
            if (fromPanel != IManagedCloudServiceOps.PanelType.None)
            {
                HidePanel(fromPanel);
            }
            
            if (toPanel != IManagedCloudServiceOps.PanelType.None)
            {
                ShowPanel(toPanel);
            }
        }
        ...
        /// </summary>
        private bool HasOtherPanelsRequiringScrim(IManagedCloudServiceOps.PanelType excludePanel)
        {
            foreach (var kvp in _uiConfig.Panels)
            {
        ...
        private void UpdateSingleStatusLabel(Label statusLabel, Label modeLabel, 
            ManagedCloudServiceInfo.ConnectionStatus status, string prefix)
        {
            ...
            switch (status)
            {
                case ManagedCloudServiceInfo.ConnectionStatus.Connected:
                    statusLabel.AddToClassList("status-connected");
                    break;
                case ManagedCloudServiceInfo.ConnectionStatus.Disconnected:
                    statusLabel.AddToClassList("status-disconnected");
                    break;
                case ManagedCloudServiceInfo.ConnectionStatus.Error:
                    statusLabel.AddToClassList("status-error");
                    break;
                case ManagedCloudServiceInfo.ConnectionStatus.Connecting:
                    statusLabel.AddToClassList("status-connecting");
                    break;
            }
        }

        private string GetStatusText(ManagedCloudServiceInfo.ConnectionStatus status)
        {
            return status switch
            {
                ManagedCloudServiceInfo.ConnectionStatus.Connected => "Connected",
                ManagedCloudServiceInfo.ConnectionStatus.Connecting => "Connecting...",
                ManagedCloudServiceInfo.ConnectionStatus.Disconnected => "Disconnected",
                ManagedCloudServiceInfo.ConnectionStatus.Error => "Error",
                _ => "Unknown"
            };
        }

        private string GetModeText(ManagedCloudServiceInfo.ConnectionStatus status)
        {
            return status switch
            {
                ManagedCloudServiceInfo.ConnectionStatus.Connected => "Active",
                ManagedCloudServiceInfo.ConnectionStatus.Connecting => "Initializing",
                ManagedCloudServiceInfo.ConnectionStatus.Disconnected => "Inactive",
                ManagedCloudServiceInfo.ConnectionStatus.Error => "Failed",
                _ => "Unknown"
            };
        }
        ...
        private void ClearAllPanelStates()
        {
            foreach (var kvp in _uiConfig.Panels)
            {
                ...
                if (panelData.Panel != null)
                {
                    ...
                    if (kvp.Key != IManagedCloudServiceOps.PanelType.NavigationMenu)
                    {
        ...
        private void OnPanelTransitionComplete(TransitionEndEvent evt)
        {
            ...
            {
                _uiConfig.SubpanelsContainer.style.display = DisplayStyle.None;
                Debug.Log("[ManagedCloudServiceUIManager] All panels closed - hiding container");
            }
        }
        ...
        /// </summary>
        public IManagedCloudServiceOps.PanelType CurrentActivePanel => _currentActivePanel;
